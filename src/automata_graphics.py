...
from src.misc import DETERMINISTIC, NONDETERMINISTIC, INVALID_TYPE, \
                 START_X, START_Y, MAX_X, \
                 ...
                 INVALID_DFA, MAX_STATES, STATES_EXCEEDED, INVALID_FILE_EXTENSION, \
                 exit_if

from finite_automata import FiniteAutomata, is_dfa_invalid
...
class GraphicInterface(QWidget):
    def __init__(self, path):
        ...
        with open(path, 'r') as f:
            self.automata = FiniteAutomata(f.read(), type)
            if self.automata.accept:
                exit_if(is_dfa_invalid(self.automata), INVALID_DFA)
            exit_if(len(self.automata.states) > MAX_STATES, STATES_EXCEEDED)
            self.initUI()

    def initUI(self):
        # set theme
        if self.automata.dark:
        ...
        # set title
        if self.automata.type == DETERMINISTIC:
            title_start = 'Deterministic Finite Automata: '
        elif self.automata.type == NONDETERMINISTIC:
            title_start = 'Nondeterministic Finite Automata: '
        else:
            exit_if(True, INVALID_FILE_EXTENSION)

        # start window
        self.setWindowTitle(title_start + self.automata.description)
        self.showMaximized()
    ...
    def draw_states(self, qp):
        ...
        for state in self.automata.states:
            ...
            self.draw_state(qp, int(x), int(y),
                            self.automata.finals.__contains__(state), self.automata.initial == state)
            # if nameless space is on, don't write state names
            if not self.automata.nameless:
                qp.drawText(x + HALF_SIZE_CIRCLE - STATE_ADJUST, y + HALF_SIZE_CIRCLE, state)
            x += int(2.2 * SIZE_CIRCLE)

        # draw arrows
        for (src_state, character), dest_states in self.automata.interface_delta.items():
